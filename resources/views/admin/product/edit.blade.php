...
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-12">
            <h2 class="mb-3">Sửa sản phẩm: {{ $product->ten }}</h2>
                ...
                <div class="card mb-4 shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <select class="form-select" id="danhmucid" name="danhmucid" required>
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ $product->danhmucid == $category->id ? 'selected' : '' }}>
                                            {{ $category->ten }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select" id="loaisanphamid" name="loaisanphamid" required>
                                    ...
                                    @foreach($productTypes as $type)
                                        <option value="{{ $type->id }}" {{ $product->loaisanphamid == $type->id ? 'selected' : '' }}>
                                            {{ $type->ten }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select" id="thuong_hieu_id" name="thuong_hieu_id">
                                    ...
                                    @foreach($brands as $b)
                                        <option value="{{ $b->id }}" {{ $product->thuong_hieu_id == $b->id ? 'selected' : '' }}>
                                            {{ $b->ten }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="ten" name="ten" value="{{ $product->ten }}" required>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control" id="giaban" name="giaban" value="{{ $product->giaban }}" min="0" step="1000" required>
                            </div>
                            <div class="col-md-6">
                                <label for="giagiam" class="form-label">Giá giảm (VNĐ)</label>
                                <input type="number" class="form-control" id="giagiam" name="giagiam" value="{{ $product->giagiam }}" min="0" step="1000">
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="mota" name="mota" rows="4">{{ $product->mota }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-wrap gap-2" id="sizesArea">
                            @foreach($sizes as $size)
                                @php
                                    $isChecked = $product->variants->pluck('kichcoid')->unique()->contains($size->id);
                                @endphp
                                <div class="form-check">
                                    <input class="form-check-input size-checkbox" type="checkbox" 
                                           id="size_{{ $size->id }}" value="{{ $size->id }}" 
                                           data-size-name="{{ $size->ten }}" {{ $isChecked ? 'checked' : '' }}>
                ...
                <div class="card mb-4 shadow-sm">
                    ...
                    <div class="card-body">
                        <div class="form-check form-switch mb-3">
                            <input class="form-check-input" type="checkbox" id="noibat" name="noibat" value="1" {{ $product->noibat ? 'checked' : '' }}>
                            <label class="form-check-label" for="noibat">
                                Sản phẩm nổi bật
                            </label>
                        </div>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="trangthai" name="trangthai" value="1" {{ $product->trangthai ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    existingImages.forEach(img => {
        if (img.mau_sac) {
            const colorExists = selectedColors.find(c => c.id == img.mausacid);
            if (!colorExists) {
                selectedColors.push({
                    id: img.mausacid,
                    name: img.mau_sac.ten,
                    color: img.mau_sac.ma_mau
                });
                existingColorImages[img.mausacid] = [];
            }
            existingColorImages[img.mausacid].push(img);
        }
    });
    
    // Load existing sizes from variants
    const uniqueSizes = [...new Set(existingVariants.map(v => v.kichcoid))];
    uniqueSizes.forEach(sizeId => {
        const variant = existingVariants.find(v => v.kichcoid == sizeId);
        if (variant && variant.kich_co) {
            selectedSizes.push({
                id: sizeId,
                name: variant.kich_co.ten
            });
        }
    });
    ...
    existingVariants.forEach(variant => {
        const key = `${variant.mausacid}_${variant.kichcoid}`;
        variantData[key] = {
            gia: variant.gia,
    ...
    function renderColorsList() {
        ...
        selectedColors.forEach((color, index) => {
            ...
            const existingImgs = existingColorImages[color.id] || [];
            const hasExistingImages = existingImgs.length > 0;
                ...
                <div class="card-body">
                    ...
                    ${hasExistingImages ? `
                        <div class="mb-2">
                            ...
                            <div class="d-flex flex-wrap gap-2" id="existing_${color.id}"></div>
                        </div>
                    ` : ''}
                    <div class="mb-2">
                        <label class="form-label">Thêm hình ảnh mới ${hasExistingImages ? '(tùy chọn)' : '<span class="text-danger">*</span>'}</label>
                        <input type="file" class="form-control color-images-input" 
                               data-color-id="${color.id}" accept="image/*" multiple ${hasExistingImages ? '' : 'required'}>
                    </div>
            ...
            if (hasExistingImages) {
                const existingContainer = document.getElementById(`existing_${color.id}`);
    ...
    function generateVariantsTable() {
        ...
        selectedColors.forEach(color => {
            ...
            // Try to get image from existing or new uploads
            let imagePreview = '';
            const existingImgs = existingColorImages[color.id] || [];
            ...
            selectedSizes.forEach(size => {
                ...
                const existingData = variantData[variantKey] || { gia: '', soluong: '' };
    ...
    document.getElementById('productForm').addEventListener('submit', function(e) {
        ...
        // Validate that colors have images (either existing or new)
        for (let color of selectedColors) {
            const hasExisting = existingColorImages[color.id] && existingColorImages[color.id].length > 0;
            const hasNew = colorImages[color.id] && colorImages[color.id].length > 0;
